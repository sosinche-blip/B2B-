...
git init
git add .
git commit -m "B2B operation V172"
git branch -M main
...
1. GitHub 업로드 전 `npm run verify:all`을 실행합니다.
2. Cloudflare Pages 빌드 후 모바일 주소 `https://b2b-bpt.pages.dev`에서 `V172_MOBILE_OPERATION_GUARD`가 표시되는지 확인합니다.
3. Worker 주소 `https://coupang-toss-b2b-automation.sosinche.workers.dev`에서 서버 점검이 정상인지 확인합니다.
4. 간편운영 화면의 `V172 모바일 단계 잠금판`에서 1~9단계가 보이는지 확인합니다.
5. 실제 운영 전 `단계점검표`를 내려받아 미매핑, 송장 확인필요, 서버 정리 OFF 여부를 확인합니다.
