...
console.log("[VERIFY] V170 mobile mapping operation audit");
...
if (!String(pkg.version || "").includes("v170")) fail("package version is not v170");
const webPkg = JSON.parse(read("apps/web/package.json"));
if (!String(webPkg.version || "").includes("v170")) fail("web package version is not v170");
const workerPkg = JSON.parse(read("apps/worker/package.json"));
if (!String(workerPkg.version || "").includes("v170")) fail("worker package version is not v170");
if (!process.exitCode) pass("V170 package versions exist");

const app = read("apps/web/src/App.tsx");
mustInclude("App", app, [
  'APP_VERSION = "V170 모바일 매핑 운영 안정화"',
...
console.log("\n[PASS] V170 service verification completed.");
