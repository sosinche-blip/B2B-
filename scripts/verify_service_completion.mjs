...
console.log("[VERIFY] V172 mobile gateway and mapping audit");
...
if (!String(pkg.version || "").includes("v172")) fail("package version is not v172");
const webPkg = JSON.parse(read("apps/web/package.json"));
if (!String(webPkg.version || "").includes("v172")) fail("web package version is not v172");
const workerPkg = JSON.parse(read("apps/worker/package.json"));
if (!String(workerPkg.version || "").includes("v172")) fail("worker package version is not v172");
if (!process.exitCode) pass("V172 package versions exist");

const app = read("apps/web/src/App.tsx");
mustInclude("App", app, [
  'APP_VERSION = "V172 모바일 게이트웨이·매핑 안정화"',
...
console.log("\n[PASS] V172 service verification completed.");
