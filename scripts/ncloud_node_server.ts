...
const server = createServer(async (incoming, outgoing) => {
  try {
    ...
    if (reqUrl.pathname.startsWith("/api/local/")) {
      const response = jsonNodeResponse({
        ok: false,
        message: "V193 Ncloud는 고정 IP 마켓 API 실행과 쿠폰 스케줄 실행만 담당하며 파일·UI를 저장하지 않습니다.",
      }, 410);
...
server.listen(port, host, () => {
  console.log(`[NCLOUD] V193 fixed-IP gateway listening on http://${host}:${port}`);
  console.log("[NCLOUD] No UI or B2B file storage is enabled.");
  console.log(`[NCLOUD] Health check: http://127.0.0.1:${port}/api/system/status`);
});
