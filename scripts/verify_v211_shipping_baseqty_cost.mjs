...
must(worker.includes('qty: Math.max(1, Math.floor(Number(order.qty || order.quantity || 1) || 1))') && worker.includes('baseQty를 다시 곱하지 않습니다'),'marketplace order qty is sent once and baseQty is not multiplied twice');
