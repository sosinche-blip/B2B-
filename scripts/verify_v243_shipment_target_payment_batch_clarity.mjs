...
must(
  worker.includes("const preparingRetry = await adminplusEnsureMarketplacePreparing") ||
  (worker.includes("const preparingRetry = { attempted: 0") && worker.includes("alreadyPrepared: marketplacePreparing.total")),
  "shipment run either retries preparing under legacy flow or starts from current marketplace preparing source"
);
...
must(
  worker.includes("송장보유 미등록 ${Number(target.trackingReadyBefore") ||
  (worker.includes("마켓 현재 상품준비중") && worker.includes("송장보유 등록대상 ${Number(target.registrationTarget")),
  "operator message separates tracking-ready or current-preparing registration target"
);
...
must(
  (worker.includes("준비전환 신규") && worker.includes("이미 준비중 확인")) ||
  (worker.includes("마켓 현재 상품준비중") && worker.includes("현재 상품준비중 외 주문은 자동 제외")),
  "shipment message reflects legacy transition flow or current-preparing source flow"
);
