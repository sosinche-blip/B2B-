# B2B 쿠팡·토스 모바일 운영 자동화 앱 V178 배포 안내
...
V178은 사용자가 첨부한 매핑 엑셀 양식 형태를 표준으로 반영하고, 모바일 화면에서 발생한 HTTP 502 진단 오류를 쿠팡·토스 API 오류와 Worker/Tunnel/Ncloud 경로 오류로 분리하기 위한 버전입니다.
...
지원 파일은 `.xlsx`, `.xls`, `.csv`입니다. V178은 CDN 엑셀 라이브러리 호출이 실패해도 단순 xlsx 구조를 앱 자체에서 읽는 보조 파서를 포함합니다.
...
1. Cloudflare Worker가 최신 V178로 배포되었는지
...
2. 화면 버전 `V178_MAPPING_UPLOAD_STABILITY_AND_NCLOUD_PROXY_GUARD` 확인
3. 매핑관리에서 V178 매핑양식 다운로드
...
Cloudflare Worker 변수에 다음 값을 유지하거나 추가합니다.

```text
NCLOUD_API_BASE=https://현재-살아있는-터널.trycloudflare.com
NCLOUD_DIRECT_API_BASE=http://101.79.27.234:8080
NCLOUD_DIRECT_FALLBACK_ENABLED=true
```

`NCLOUD_API_BASE`가 임시 Tunnel이라도, Ncloud 8080 포트가 외부에서 열려 있으면 V178 Worker는 `NCLOUD_DIRECT_API_BASE`로 한 번 더 중계합니다.

Ncloud 서버에서 확인할 명령은 다음입니다.
...
```bash
curl -s http://101.79.27.234:8080/api/system/status
curl -s http://101.79.27.234:8080/api/system/public-ip
```
